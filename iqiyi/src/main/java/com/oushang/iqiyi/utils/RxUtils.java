...
public class RxUtils {
    ...
    public  <T> Disposable executeTimeOut(long start, long count, long initialDelay, long period, Function<Long, Observable<T>> run, Function<T, Boolean> takeUtil, Consumer<T> result) {
        return Observable.intervalRange(start, count, initialDelay, period, TimeUnit.MILLISECONDS)
                .flatMap(run)
                .takeUntil(takeUtil::apply)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
